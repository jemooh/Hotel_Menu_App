...
@SuppressLint("HandlerLeak")
		...
		public class ListMainCoursesFragment extends Fragment implements FoodClickListener {
		    ...
		    public void getItemsFeed(View v){
	    	   ...
	    	   new Thread(new GetSavedFoodsTask(responseHandler2,   null,null)).start();
	    	
	    	   }else new Thread(new GetFoodsTask(responseHandler,   null,null)).start();} 
		    ...
		    @SuppressLint("HandlerLeak")
			    ...
			    private void populateListWithItems2(Message msg) {
			        ...
			        FoodLibrary lib = (FoodLibrary) msg.getData().get(GetSavedFoodsTask.LIBRARY);
		    ...
		    private void populateListWithItems(Message msg) {
		        // Retreive the videos are task found from the data bundle sent back
		        FoodLibrary lib = (FoodLibrary) msg.getData().get(GetFoodsTask.LIBRARY);
		        // Because we have created a custom ListView we don't have to worry about setting the adapter in the activity
		        // we can just call our custom method with the list of items we want to display
		        ProgressBar pbpp = (ProgressBar) getView().findViewById(R.id.pbppl);
		        TextView txtMsg = (TextView) getView().findViewById(R.id.progressMsg2);
		        listView.setItems(lib.getfoods());
		        String jsonString = GetFoodsTask.json;
		        Log.i("jsonString", jsonString);
		        dbHandler.addMainfood(jsonString);
				if(lib.getfoods().isEmpty()){
					txtMsg.setText("No foods on the menu");	
					pbpp.setVisibility(View.GONE);
				} else {
